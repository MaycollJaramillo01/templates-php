...
if (!function_exists('home4_load_testimonials')) {
    function home4_load_testimonials(string $file): array
    {
        if (!file_exists($file)) {
            return [];
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        $current = home4_load_testimonials($testimonialFile);
        $current[] = $entry;
...
$testimonials = home4_load_testimonials($testimonialFile);
...
<main id="main-content" class="section section-light">
  <div class="container stack">
    <header data-anim="from-bottom">
      ...
      <p class="text-muted">See why homeowners and property managers trust <?php echo htmlspecialchars($Company); ?>.</p>
    </header>
    <?php if ($success): ?>
    ...
    <?php endif; ?>
    <section class="grid grid-2" aria-label="Testimonials form">
      <form class="card stack" method="post" data-anim="from-bottom">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
        <label for="t4-name">Name</label>
        <input id="t4-name" name="name" type="text" required>
        <label for="t4-email">Email <span class="text-muted">(optional)</span></label>
        <input id="t4-email" name="email" type="email">
        <label for="t4-rating">Rating</label>
        <select id="t4-rating" name="rating" required>
          <option value="">Select rating</option>
          <?php for ($i = 5; $i >= 1; $i--): ?>
            <option value="<?php echo $i; ?>"><?php echo $i; ?> star<?php echo $i > 1 ? 's' : ''; ?></option>
          <?php endfor; ?>
        </select>
        <label for="t4-comment">Comments</label>
        <textarea id="t4-comment" name="comment" rows="4" required></textarea>
        <div class="sr-only">
          <label for="t4-website">Website</label>
          <input id="t4-website" type="text" name="website" tabindex="-1" autocomplete="off">
        </div>
        <button class="btn btn-primary" type="submit" data-anim="from-bottom" data-anim-delay="120">Submit review</button>
      </form>
      <aside class="card" data-anim="from-left">
        <h2>Share your story</h2>
        <p>We celebrate every success and address any concerns immediately.</p>
      </aside>
    </section>
    <section aria-label="Testimonials list" class="stack">
      <h2 data-anim="from-bottom">Recent reviews</h2>
      <div class="grid" style="gap:var(--spacing-4);">
        <?php if (!$visible): ?>
          <p data-anim="from-bottom">No reviews yet. Add yours now.</p>
        <?php else: ?>
